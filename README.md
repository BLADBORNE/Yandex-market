...
идемпотентности. Недостаток средств удаляет ожидающий заказ и возвращает корзину
в `ACTIVE` даже после потерянного более раннего ответа: при успешном первом
списании идемпотентный повтор с тем же ключом вернул бы сохранённый успех,
поэтому такой отказ однозначно означает, что списания не было. Ошибка валидации
или OAuth2 размораживает корзину только до неоднозначной попытки; после
потерянного ответа платёжный intent сохраняется до подтверждения результата.
Новая покупка после безопасной отмены получает новый ключ.
`IDEMPOTENCY_CONFLICT` также считается неопределённым результатом и не
размораживает корзину автоматически. Если процесс аварийно завершился,
